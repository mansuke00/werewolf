...
// 長押しで中身が見える仕様（覗き見防止）
export const MiniRoleCard = ({ role, teammates }) => {
  ...
  // 仲間表示が無い役職リスト
  const hiddenTeammateRoles = ['citizen', 'killer', 'cursed', 'elder'];
  ...
  return (
      ...
      >
          <div className={`transition-all duration-200 w-full ${revealed ? "blur-0 opacity-100" : "blur-md opacity-50"}`}>
             ...
             <h3 className="text-xl font-black text-white mb-1">{roleDef?.name}</h3>
             {revealed && (
