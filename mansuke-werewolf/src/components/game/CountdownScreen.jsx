    ...
    return (
        <div className={`fixed inset-0 z-[100] flex flex-col items-center justify-center overflow-hidden transition-colors duration-1000 ease-out ${style.bg}`}>
            ...
            <div className="relative z-20 flex flex-col items-center justify-center">
                <span 
                    key={count}
                    className={`text-[35vh] md:text-[45vh] font-bold leading-none tracking-tighter drop-shadow-2xl select-none font-mono transition-colors duration-300 ${style.text} ${count > 0 ? "animate-scale-fade" : ""}`}
                >
                    {count}
                </span>
            </div>
            ...
            {/* 警告通知（カウントダウン下部に移動） */}
            <div className="absolute bottom-32 md:bottom-40 left-0 w-full z-30 flex justify-center">
                <div className="bg-red-900/40 border border-red-500/50 px-8 py-4 rounded-full backdrop-blur-md animate-pulse shadow-[0_0_30px_rgba(220,38,38,0.4)] max-w-md mx-4 text-center">
                    <p className="text-base md:text-lg text-red-200 font-bold flex items-center justify-center gap-2 tracking-wider">
            ...
            {/* 下部情報バー */}
            <div className="absolute bottom-0 w-full bg-black/60 backdrop-blur-lg border-t border-white/5 p-8 flex justify-between items-center z-30">
                <div className="flex gap-12 opacity-60">
                    <div className="flex flex-col">
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.3em] flex items-center gap-2 mb-1"><Hash size={10}/> Room Code</span>
                        <span className="text-2xl font-mono font-bold text-white tracking-widest">{roomCode}</span>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.3em] flex items-center gap-2 mb-1"><Fingerprint size={10}/> Match ID</span>
                        <span className="text-2xl font-mono font-bold text-white tracking-widest">{matchId}</span>
                    </div>
                </div>
